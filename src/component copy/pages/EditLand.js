...
const EditLand = () => {
  ...
  console.log(landData[0][0], "landdetails");
  const EditLands = landData[0][0];
  const availabelLeaseOwnerIdList = [];
  const availableLeaseLandIdList = [];
  const availableLeaseFarmers = [];
  const availabelLeaseList = {};

  const [area, setArea] = useState(EditLands.area);
  const [interestedFor, setInterestedFor] = useState(EditLands.category);
  const [addOns, setAddOns] = useState(EditLands.addons);
  const [supervisorID, setSupervisorID] = useState(EditLands.supervisorId);
