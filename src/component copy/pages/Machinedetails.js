...
const Machinedetails = () => {
    ...
    const saveHandler = async (e) => {
        ...
        // console.log("current",machine_detail[i]);
        if (machine_detail[i]["enteredtype"] === enteredType){
            output.push(inputdata)
        }
        else{
            output.push(machine_detail[i])
        }
       }
       console.log("output",output);
       {create ? dispatch(farmerActions.create_machine_detail(inputdata)) : dispatch(farmerActions.create_machine_detail(output))}
    //    navigate("/viewmachine");
        
        // {create ? await axios.post(`${api}/machinery/create`,{ machineDetails : [inputdata] })
        // .then((response) => {
    ...
    return (
        ...
        <section className={classes.box}>
            ...
            {create ? <select name="subtype" id="subtype" value={enteredSubtype} onChange={(e) => setEnteredSubtype(e.target.value)}>
                    <option value="subtype">Sub Type</option>
                    {Subtype.map( (x,y) => 
                    <option key={y} value={x}>{x}</option> )}
            </select> : <select name="subtype" id="subtype" value={enteredSubtype} onChange={(e) => setEnteredSubtype(e.target.value)}>
                    <option value="subtype">{enteredSubtype}</option>
                    {Subtype.map( (x,y) => 
                    <option key={y} value={x}>{x}</option> )}
            </select>}
            {create ? <select name="attach" id="attach" value={enteredAttachment} onChange={(e) => setEnteredAttachment(e.target.value)}>
                    <option value="attachment">Attachments</option>
                    {Attachment.map( (x,y) => 
                    <option key={y} value={x}>{x}</option> )}
            </select> : <select name="attach" id="attach" value={enteredAttachment} onChange={(e) => setEnteredAttachment(e.target.value)}>
...
const Machinedetails = () => {
    ...
    const saveHandler = async (e) => {
        ...
        //         console.log("submitting machine details");
